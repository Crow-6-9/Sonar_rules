...
class CheckstyleScraper:
    ...
    def extract_description(self, section: BeautifulSoup) -> Optional[str]:
        """Extract paragraphs, code, lists, and table text from description."""
        if not section:
            return None
        parts: List[str] = []
    ...
    def extract_examples(self, section: BeautifulSoup) -> List[str]:
        """Extract all <pre> blocks as examples."""
        examples = []
        if not section:
            return examples
    ...
    def extract_rule_details(self, rule_url: str, rule_name: str) -> Optional[Dict]:
        """Extract rule details (title, checkstyle, description, properties, examples)."""
        self.logger.info(f"Extracting rule: {rule_name}")
    ...
    def scrape(self) -> Dict:
        """Main orchestration: get rules list → extract each rule."""
        ...
        self.logger.info(f"✅ Extraction complete — {len(rules_data)} rules saved to {filename}")
        return final_json
